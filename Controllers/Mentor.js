...
const createMentor = async (req, res) => {
  try {
    ...
    } else await newMentor.save();
    res
      .status(200)
      .send({ message: "Mentors Data Created Successfully", newMentor });
  } catch (error) {
    res
      .status(500)
...
const AssignStudent = async (req, res) => {
  try {
    ...
    const existingStudent = mentor.studentsAssigned.find(
      (student) => student.studentId === req.body.StudentId
    );
    if (existingStudent) {
      return res
        .status(400)
        .send({ message: "Student already assigned to this mentor" });
    }
    ...
    res
      .status(200)
      .send({ message: "Student assigned to this mentor Successfully" });
  } catch (error) {
    res
      .status(500)
      .send({ message: "Internal Server Error", error: error.message });
